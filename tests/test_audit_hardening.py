...
def test_vouch_rejects_out_of_range_score(tmp_path, monkeypatch):
    ...
    for bad in ("1.5", "-0.1"):
        r = runner.invoke(cli, ["vouch", "--from", "alice", "--for", did,
                                "--score", bad, "--scope", "coding"])
        assert r.exit_code == 2, (bad, r.output)
        assert "score must be within [0, 1]" in r.output
    # boundary values stay legal
    for good in ("0", "1"):
        r = runner.invoke(cli, ["vouch", "--from", "alice", "--for", did,
                                "--score", good, "--scope", "coding",
                                "--out", str(tmp_path / f"v{good}.json")])
        assert r.exit_code == 0, (good, r.output)


def test_vc_export_carries_evidence():
    issuer = generate_identity()
    v = create_vouch(issuer, issuer.public_key, score=0.9, scope="coding",
                     evidence=["https://ops.example/task/42"], ts=1000)
...
def test_reissue_preserves_claim_and_evidence():
    ...
    v = create_vouch(old_key, subject.public_key, score=0.9, scope="coding",
                     claim="reviewed Q3 deliverables",
                     evidence=["ticket:ATAR-7"], ts=1000)
