...
def _seed_cli_network(tmp_path, monkeypatch):
    ...
    for frm, to, score in (("seed", "research", "0.9"),
                           ("seed", "reporting", "0.75"),
                           ("research", "reporting", "0.95")):
        vf = tmp_path / f"{frm}-{to}.json"
        assert runner.invoke(cli, ["vouch", "--from", frm, "--for", dids[to],
                                   "--score", score, "--scope", "coding",
                                   "--out", str(vf)]).exit_code == 0
...
def test_dashboard_cli_resolves_keygen_names(tmp_path, monkeypatch):
    ...
    r = runner.invoke(cli, ["dashboard", "--seed", dids["seed"],
                            "--scope", "coding", "--out", str(out)])
    assert r.exit_code == 0
    html = out.read_text()
